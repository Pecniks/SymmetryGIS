...
namespace Symmetry {
    ...
    private:
        ...
        std::vector<RotationalSymmetry> getRotationalSymmetriesAroundCenterPoint(std::vector<std::vector<Voxel>>& field, Point<double> centerPoint, const uint16_t maxRadius, int32_t pixelX, int32_t pixelY);  // Function that increments the radius of the circumference around the center point with the step of 1. In every step it selects all interesting voxels the circumference overlaps with and calls the function FindRotationalSymmetries. In the end a union of all interesting voxels from obtained symmetries with different radii is made. The function returns a list of rotational symmetries (2 through 16), where each rotational symmetry is present once at most.
        std::vector<RotationalSymmetry> mergeSymmetries(std::vector<RotationalSymmetry>& symmetries);  // Merging symmetries by layers. Symmetries with the same search axis and rotation level are merged.
        std::vector<RotationalSymmetry> getRotationalSymmetriesOnRadius(Point<double> centerPoint, const std::vector<Voxel>& interestingVoxels);  // A main function for searching symmetries gets a center point and a list of interesting voxels. Symmetries are searched for only in the set of interesting voxels that are on the same radius from the center point (or inside the same interval).
    public:
        // CONSTRUCTOR AND DESTRUCTOR
        RotationalSymmetry();   // Default constructor.
        RotationalSymmetry(const Point<double>& symmetryAxis, const uint8_t& rotation, const uint32_t& upperLayer, const uint32_t& lowerLayer);  // Constructor with all parameters.
        ~RotationalSymmetry();  // Destructor.

        // OVERLOADED OPERATORS
        RotationalSymmetry operator & (const RotationalSymmetry& s) const;  // Operator & is used for merging two symmetries.
        ...
        // OBJECT METHODS
        std::vector<RotationalSymmetry> getRotationalSymmetries(std::vector<std::vector<std::vector<Voxel>>> voxels, VoxelMesh& voxelMesh);  // Finding rotational symmetries.
