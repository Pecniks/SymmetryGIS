...
#include "bruterotationalsymmetry.h"
...
std::vector<BruteRotationalSymmetry> BruteRotationalSymmetry::mergeSymmetries(std::vector<BruteRotationalSymmetry>& symmetries) {
    ...
    for (BruteRotationalSymmetry& symmetry : symmetries) {
        ...
        // Updating the Symmetry object with new limits.
            ...
            const int index = std::distance(mergedSymmetries.begin(), itr);
    ...
    const std::vector<Voxel>& interestingVoxels)
{
    ...
    for (uint32_t i = 2; i <= maxSymmetrySize; i++) {
        ...
        const int layer = centerPoint.getZ();
...
Voxel BruteRotationalSymmetry::getVoxel(int index) const {
    return voxels[index];
}
...
Point<double> BruteRotationalSymmetry::getSymmetryAxis() const {
    return symmetryAxis;
}

// Setting a symmetry axis.
void BruteRotationalSymmetry::setSymmetryAxis(Point<double> symmetryAxis) {
    this->symmetryAxis = symmetryAxis;
}

// Getting a rotation.
uint8_t BruteRotationalSymmetry::getRotation() const {
    return rotation;
}

// Setting a rotation.
void BruteRotationalSymmetry::setRotation(uint8_t rotation) {
    this->rotation = rotation;
}

// Getting an upper layer.
uint32_t BruteRotationalSymmetry::getUpperLayer() const {
    return upperLayer;
}

// Setting an upper layer.
void BruteRotationalSymmetry::setUpperLayer(uint32_t upperLayer) {
    this->upperLayer = upperLayer;
}

// Getting a lower layer.
uint32_t BruteRotationalSymmetry::getLowerLayer() const {
    return lowerLayer;
}

// Setting a lower layer.
void BruteRotationalSymmetry::setLowerLayer(uint32_t lowerLayer) {
...
void BruteRotationalSymmetry::setGeometrySearch(int32_t x, int32_t y) {
...
void BruteRotationalSymmetry::clearGeometrySearch(int32_t x, int32_t y) {
...
void BruteRotationalSymmetry::getDistanceRanges(Point<double> centerPoint, Voxel v, double& minD, double& maxD) {
    minD = 1000000.0;
    maxD = 0.0;
...
float BruteRotationalSymmetry::getAngle(const Point<double> a, const Point<double> b) {
...
std::vector<PositionFromPlane> BruteRotationalSymmetry::calculatePositionsFromAxis(const Point<double>* axis) {
    ...
    for (unsigned long long i = 0; i < numberOfPoints; i++) {
        ...
        const int voxelX = floor((point.getX() - voxelMesh.minX) / voxelMesh.voxelSideSize);
        const int voxelY = floor((point.getY() - voxelMesh.minY) / voxelMesh.voxelSideSize);
        const int voxelZ = floor((point.getZ() - voxelMesh.minZ) / voxelMesh.voxelSideSize);
